...
throughput. We currently request **1 GPU** (`SLURM_GPUS=1`); cluster a100 nodes have
...
- **Risks:** (a) double-backward × DDP allreduce correctness — test it; (b) size-grouped
  batch-count desync hang — the min-count sync is the fix; (c) NCCL/module setup on cluster.
