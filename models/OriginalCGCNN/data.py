    ...
    def __getitem__(self, idx):
        ...
        def get_occu(crystal_structure):
            ...
            for i in range(len(periodic_sites['sites'])):
                site_fraction = []
                site_species = []
                for d in periodic_sites['sites'][i]['species']:
                    site_fraction.append(d['occu'])
                    site_species.append(Element(d['element']).Z)
                # print(site_fraction, site_species)
                crystal_fractions.append(site_fraction)
                crystal_species.append(site_species)
            return crystal_fractions, crystal_species

        crystal_fractions, crystal_species = get_occu(crystal)
        # print('crystal_fractions', crystal_fractions)
        # print('crystal_species', crystal_species)

        atom_fea_crystal = []

        site_number = 0
        for site in crystal_species:
            # print('site', site)
            dopants = 0
            vector_length = len(self.ari.get_atom_fea(1))
            elementWise_sum = [0] * vector_length
            for i in site:
                atom_fea_site = self.ari.get_atom_fea(i)
                # print('atom_fea_site before scaling', atom_fea_site)
                scaler = crystal_fractions[site_number][dopants]
                atom_fea_site = atom_fea_site * scaler
                # print('scalar', scaler)
                # p#rint('scaling by: ', scaler, 'SHAPE', atom_fea_site.shape, 'atom_fea_site after scaling', atom_fea_site)
        ...
        # print('atom_fea_crystal', atom_fea_crystal)

        ##########
        atom_fea = torch.Tensor(np.array(atom_fea_crystal))  # changed to numpy.array)
        # print("shape: ", atom_fea.shape)
