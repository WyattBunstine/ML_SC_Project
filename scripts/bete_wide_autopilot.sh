...
STATUS=model_data/cf_calib/bete_wide_status.txt
note() { echo "$(date '+%m-%d %H:%M') $*" >> "$STATUS"; }
die()  { note "HALT: $*"; exit 1; }
: > "$STATUS"
note "bete-wide autopilot started (rung 47: rung-42 recipe at 256-dim latents (capacity test))"
...
CKPT="${RDIR}/result_t47_model_best.pth.tar"
ssh "$SSH" "[ -f '${RP}/${CKPT}' ]" || die "checkpoint missing remotely (${CKPT})"

for HC in configs/head/gps_tc_probe_47wide.json configs/head/gps_tc_probe_47wide_l1k.json \
          configs/head/gps_tc_la_series_47wide.json configs/head/gps_tc_probe_47wide_g_l1k.json \
          configs/head/gps_tc_la_series_47wide_g.json configs/head/gps_tc_la_series_47wide_lg.json; do
...
HJ=""
OUT=$(./scripts/deploy.sh run-head configs/head/gps_tc_probe_47wide.json \
      configs/head/gps_tc_probe_47wide_l1k.json configs/head/gps_tc_la_series_47wide.json 2>&1)
J1=$(echo "$OUT" | grep -oE 'Submitted batch job [0-9]+' | grep -oE '[0-9]+')
[ -n "$J1" ] && { note "baseline head batch: job $J1"; HJ="$HJ $J1"; }
OUT=$(./scripts/deploy.sh run-head configs/head/gps_tc_probe_47wide_g_l1k.json \
      configs/head/gps_tc_la_series_47wide_g.json configs/head/gps_tc_la_series_47wide_lg.json 2>&1)
...
HEAD_BATCH_NAMES="probe_47wide probe_47wide_l1k la_series_47wide probe_47wide_g_l1k la_series_47wide_g la_series_47wide_lg" \
