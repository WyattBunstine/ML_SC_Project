...
# constants; then site-pheasy -> bake -> pack -> sync to Rockfish scratch.
# Every stage is checked by its own completion line (overnight-autonomy rule).
#   systemd-run --user ... bash -c './scripts/phdos_site_autopilot.sh >> model_data/cf_calib/phdos_site_autopilot.out 2>&1'
cd "$(dirname "$0")/.." || exit 1
...
python scripts/build_phdos_site_corpus.py site-togo --workers 4
...
python scripts/build_phdos_site_corpus.py site-pheasy --workers 4
...
python scripts/build_phdos_site_corpus.py bake || die "bake failed"
note "bake: $(grep '^bake:' model_data/cf_calib/phdos_site_autopilot.out | tail -1)"
python scripts/build_phdos_site_corpus.py report >> model_data/cf_calib/phdos_site_autopilot.out 2>&1
...
R=/scratch4/tmcquee2/wbunsti1/ML_SC_Proj/MP_PhononDOS/phdos_pack_v45_site
ssh wbunsti1@login.rockfish.jhu.edu "mkdir -p '$R'" && rsync -a --partial "$OUT/" "wbunsti1@login.rockfish.jhu.edu:$R/" || die "sync failed"
