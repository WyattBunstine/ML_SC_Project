#!/bin/bash
# One-shot schema-2 consistency wave (2026-07-28): rebuild all local CF graph
# sets with the fixed physics (RPTF d57ebf7), repack, verify, sync, replace the
# corrupted scratch DOS pack, kick the MPtrj --force re-augment on the cluster.
# Explicit && chaining throughout — `set -e` is unreliable under the tool's
# exit-code wrapper (that is HOW the corrupted pack got packed).
cd "$(dirname "$0")/.." || exit 1
LOG=model_data/cf_calib/schema2_wave.log
{
echo "=== 1. DOS graphs (schema-aware resume re-bakes pre-fix survivors) ===" && \
python scripts/rebuild_dos_graphs_cf.py --build && \
echo "=== 2. SC graphs ===" && \
