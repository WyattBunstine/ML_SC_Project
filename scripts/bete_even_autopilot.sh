...
STATUS=model_data/cf_calib/bete_even_status.txt
note() { echo "$(date '+%m-%d %H:%M') $*" >> "$STATUS"; }
die()  { note "HALT: $*"; exit 1; }
: > "$STATUS"
note "bete-wide autopilot started (rung 48: rung-42 recipe + member_weights 1,1,16 (balanced a2f sampling, weight back to 1))"

ARM="48_bete_evensample"
...
CKPT="${RDIR}/result_t48_model_best.pth.tar"
ssh "$SSH" "[ -f '${RP}/${CKPT}' ]" || die "checkpoint missing remotely (${CKPT})"

for HC in configs/head/gps_tc_probe_48even.json configs/head/gps_tc_probe_48even_l1k.json \
          configs/head/gps_tc_la_series_48even.json configs/head/gps_tc_probe_48even_g_l1k.json \
          configs/head/gps_tc_la_series_48even_g.json configs/head/gps_tc_la_series_48even_lg.json; do
...
HJ=""
OUT=$(./scripts/deploy.sh run-head configs/head/gps_tc_probe_48even.json \
      configs/head/gps_tc_probe_48even_l1k.json configs/head/gps_tc_la_series_48even.json 2>&1)
J1=$(echo "$OUT" | grep -oE 'Submitted batch job [0-9]+' | grep -oE '[0-9]+')
[ -n "$J1" ] && { note "baseline head batch: job $J1"; HJ="$HJ $J1"; }
OUT=$(./scripts/deploy.sh run-head configs/head/gps_tc_probe_48even_g_l1k.json \
      configs/head/gps_tc_la_series_48even_g.json configs/head/gps_tc_la_series_48even_lg.json 2>&1)
...
HEAD_BATCH_NAMES="probe_48even probe_48even_l1k la_series_48even probe_48even_g_l1k la_series_48even_g la_series_48even_lg" \
