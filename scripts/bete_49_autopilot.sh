...
STATUS=model_data/cf_calib/bete_49_status.txt
note() { echo "$(date '+%m-%d %H:%M') $*" >> "$STATUS"; }
die()  { note "HALT: $*"; exit 1; }
: > "$STATUS"
note "bete-wide autopilot started (rung 49: wide recipe + per-atom site-projected phonon DOS task)"
...
CKPT="${RDIR}/result_t49_model_best.pth.tar"
ssh "$SSH" "[ -f '${RP}/${CKPT}' ]" || die "checkpoint missing remotely (${CKPT})"

for HC in configs/head/gps_tc_probe_49site.json configs/head/gps_tc_probe_49site_l1k.json \
          configs/head/gps_tc_la_series_49site.json; do
...
HJ=""
OUT=$(./scripts/deploy.sh run-head configs/head/gps_tc_probe_49site.json \
      configs/head/gps_tc_probe_49site_l1k.json configs/head/gps_tc_la_series_49site.json 2>&1)
...
HEAD_BATCH_NAMES="probe_49site probe_49site_l1k la_series_49site" \
