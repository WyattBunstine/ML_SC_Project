...
def gen_dataset(min_band_gap=4.0, prop_file=None, cif_loc=None, limit=None, chunk_size=1000):
    ...
    prop_file : str or None output id->property CSV
                            (default: database/datafiles/Non_SC_DB_MP/Non_SC.csv)
    cif_loc : str or None   output CIF directory
                            (default: database/datafiles/Non_SC_DB_MP/cifs/)
    ...
    data_dir = os.path.join(here, "datafiles", "Non_SC_DB_MP")
    if prop_file is None:
        prop_file = os.path.join(data_dir, "Non_SC.csv")
    if cif_loc is None:
