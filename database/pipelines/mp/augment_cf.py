...
Atomic: writes via tmp + os.replace so a killed run never corrupts a graph.
  ...
  python database/pipelines/mp/augment_cf.py --graph-dir <dir> [--workers N] [--limit N]
  python database/pipelines/mp/augment_cf.py --index <index.pickle>   # augment referenced graphs
"""
import argparse
import glob
import json
