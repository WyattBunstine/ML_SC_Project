...
|-----|------|-------------|
| `index_path` | string | Path to the index pickle produced by `build-db --kind cgv4` (e.g. `database/MP/id_prop_v4.pickle`). Each row points at a per-material graph JSON. |
...
```json
{
    "index_path":    "database/MP/id_prop_v4.pickle",
...
```json
{
    "index_path":         "database/MP_Energy/id_prop_v4_energy.pickle",
